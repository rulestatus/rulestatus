...
export async function loadConfig(configPath?: string): Promise<RulestatusConfig> {
  ...
  return {
    ...
    reporting: {
      format: Array.isArray(rp["format"])
        ? (rp["format"] as string[])
        : defaults.reporting.format,
      outputDir: String(rp["output_dir"] ?? rp["outputDir"] ?? defaults.reporting.outputDir),
      badge: Boolean(rp["badge"] ?? defaults.reporting.badge),
    },
    severityGate: {
      failOn: String(sg["fail_on"] ?? sg["failOn"] ?? defaults.severityGate.failOn) as RulestatusConfig["severityGate"]["failOn"],
      warnOn: String(sg["warn_on"] ?? sg["warnOn"] ?? defaults.severityGate.warnOn) as RulestatusConfig["severityGate"]["warnOn"],
    },
  };
}
